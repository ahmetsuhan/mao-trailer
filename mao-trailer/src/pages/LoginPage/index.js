...
import "../../styles/LoginPage.style.scss";
...
const LoginPage = ({ setCurrentUser }) => {
  ...
  const handleFormSubmit = (e) => {
    ...
    if (loggedInUser) {
      const userId = loggedInUser.id.value;
      handleLoginRedirect(userId, history);
      setCurrentUser({ ...loggedInUser, loggedIn: true });
    }
  };

  const handleLoginRedirect = (userId, history) => {
  ...
  const isLoginSuccess = (options, callback) => {
    let email = options.email ?? "";
    let password = options.password + "" ?? "";
    let user = options.users ?? [];
    ...
    if (!isUserPasswordValid(password, user)) {
      return false;
    }
  ...
  const isUserPasswordValid = (password = "asd", user) => {
